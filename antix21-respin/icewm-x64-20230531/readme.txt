...
NOTE:
xxxxxx.iso is set NO "boot parameters", which is differ from the last release.

You can choose any locale from among pull-down list.
...
antiX-22_***-base.iso "Grup Yorum" 19 Oct 2022
...
To switch from direct mode to uim input, set as follows:
Main menu (antiX) --> Applications --> Preferences --> Input Method (uim) --> Global Settings --> check "Specify default IM --> select Anthy (UTF-8) --> OK --> Restart Session
...
Then, press Shift + Space key to enable Japanese input.
